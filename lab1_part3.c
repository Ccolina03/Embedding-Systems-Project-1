/*
 * Lab 1, Part 3 – Additional Peripheral Interfaces
 *
...
static void HandleC4Command(Message* message);
/*****************************************************************************/
...
/*************************** Enter your code here ****************************/
		...
		while (xQueueReceive(xGreenLedQueue, &message, 0) == pdFALSE);
/*****************************************************************************/
		switch(message.type){
            case 'a': // set the green LEDs to the values of the switches
				greenLedsValue = XGpio_DiscreteRead(&switchGpio, 2);
				xil_printf("%d", greenLedsValue);
				break;

            case 's':
            	  do {
            	        buttonValue = XGpio_DiscreteRead(&buttonGpio, 1);
            	        xil_printf("%d the value\n", buttonValue);
            	        vTaskDelay(pdMS_TO_TICKS(50));
            	    } while (buttonValue != BTN1 &&  buttonValue != BTN2);

            	    xil_printf("BTN1 Pressed!\n");
            	    int tickTimeLoop = 0;
            	    if (buttonValue == BTN1) {
            	    	 while (tickTimeLoop < 50) {
            	    	    tickTimeLoop++;
            	    	    greenLedsValue = randSw[rand() % 4]; // Set random value
            	    ...
            	    if (buttonValue == BTN2) {
            	    	 ...
            	    	 while (tickTimeLoop < 50) {
            	    	     tickTimeLoop++;
            	    	     i = i % 4;
            	    	     greenLedsValue = randSw[i]; // Set random value
            	    	     XGpio_DiscreteWrite(&greenLedGpio, 1, greenLedsValue);
            	    	     vTaskDelay(pdMS_TO_TICKS(50));
            	    	     i++;
            	    	}
...
/*************************** Enter your code here ****************************/
    ...
    xQueueSend(xGreenLedQueue, message, 0); //0?
/*****************************************************************************/
